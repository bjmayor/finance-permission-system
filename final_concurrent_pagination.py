...
def get_user_permissions(supervisor_id: int) -> Dict:
    """获取用户权限数据（带缓存）"""
    cache_key = f"permissions_{supervisor_id}"

    # 先检查缓存
    cached_permissions = memory_cache.get(cache_key)
    ...
    try:
        ...
        # 缓存权限数据
        memory_cache.set(cache_key, permissions_data)
...
def get_paginated_financial_data(supervisor_id: int, page: int = 1, page_size: int = 20,
    ...
    return {
        ...
        "optimization": {
            "method": "parallel_query_with_memory_cache",
...
def simulate_concurrent_load(num_users: int = 10, requests_per_user: int = 5):
    ...
    print(f"缓存命中: {len(cache_hits)}/{len(successful_requests)} ({len(cache_hits)/len(successful_requests)*100:.1f}%)")
...
def main():
    ...
    # 小规模并发测试 - 模拟20个用户
    simulate_concurrent_load(num_users=20, requests_per_user=2)

    print("\n" + "="*60 + "\n")

    # 中等规模并发测试 - 模拟50个用户
    simulate_concurrent_load(num_users=50, requests_per_user=1)
